...
//文本数据包是字符串，不方便修改，所以直接用发送函数String
char Serial_RxPacket[100];//接收数据包载荷，接收指令限制100个字符
...
void USART1_IRQHandler(void)
{
	...
	if(USART_GetFlagStatus(USART1, USART_FLAG_RXNE) == SET)
	{
		...
		if(RxState == 0 && Serial_RxFlag == 0)//等待包头
		{
			//删除（读取RXFlag标志位并清零）的函数，在main函数读取RxPacket完毕后清零，保证读取完毕后再接收数据，缺点是读取速度不能太快
			if(RxData == '@')
			{
				RxState = 1;
			}
		...
		else if(RxState == 1)//接收数据,遇到包尾，状态由1->2
		{
			if(RxData == '\r')
			{
				RxState = 2;
