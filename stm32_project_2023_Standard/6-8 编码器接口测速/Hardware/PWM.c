...
/**
  * @brief  对定时器2的输出比较模式通道1初始化：单位时间间隔（）s
  * @param  无
  * @retval 无
  */
void PWM_Init(void)
{
	...
	//将AP0配置为复用推挽输出，其控制权由输出数据寄存器转移给输出比较模式下的定时器，由手动变自动
...
/**
  * @brief  设置定时器2通道1的频率 ：ARR 固定 100 - 1，方便计算占空比，所以只能调节Psc来改变频率
  * @param  Prescaler：72M/100/Prescaler
  * @retval 无
  */
void PWM_SetPrescaler(uint16_t Prescaler)
{
	TIM_PrescalerConfig(TIM2, Prescaler, TIM_PSCReloadMode_Update);
	//TIM_PSCReloadMode_Immediate：是让重新设置的频率立刻生效 
	//TIM_PSCReloadMode_Update：等更新事件即上一个周期结束再生效
}
