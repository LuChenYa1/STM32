...
void IC_Init(void)
{
	...
	TIM_TimeBaseInitStructure.TIM_Period = 65536 - 1;                //周期，自动重装值ARR,调到最大，防止测较低频率时计数溢出
	...
	TIM_ICInitStructure.TIM_ICFilter = 0x4;//滤波选择最大，尽量去毛刺
	...
	//FP2测占空比
//	TIM_ICInitStructure.TIM_Channel = TIM_Channel_2;//TIM3_CH2
//	TIM_ICInitStructure.TIM_ICFilter = 0xF;//滤波选择最大，尽量去毛刺
//	TIM_ICInitStructure.TIM_ICPolarity = TIM_ICPolarity_Falling;//下降沿触发
//	TIM_ICInitStructure.TIM_ICPrescaler = TIM_ICPSC_DIV1;//一分频
//	TIM_ICInitStructure.TIM_ICSelection = TIM_ICSelection_IndirectTI;//交叉通道，CH1——>TI1FP2

	TIM_PWMIConfig(TIM3, &TIM_ICInitStructure);
	//自动把CHX对应的剩下的通道配置成相反的配置，滤波、分频不变，其他变成通道2、下降沿、交叉通道
...
/**
  * @brief  读取当前输入信号波形的频率
  * @param  无
  * @retval TIM_GetCapture2(TIM3) * 100 / TIM_GetCapture2(TIM3);
  */
uint32_t IC_GetDuty(void)
{
	return (TIM_GetCapture2(TIM3) + 1) * 100 / (TIM_GetCapture1(TIM3) + 1);//放大100倍，变成百分比形式
}
